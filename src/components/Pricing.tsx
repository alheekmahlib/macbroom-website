...
interface Plan {
  ...
  cta: string;
  highlighted: boolean;
  popular?: string; // billing period that's "most popular"
}
...
function getPriceDisplay(
  ...
  const displayAmount = hasDiscount ? `$${price.discounted!.toFixed(2)}` : `$${price.original.toFixed(2)}`;
  const saved = hasDiscount ? Math.round((1 - price.discounted! / price.original) * 100) : null;

  return { amount: displayAmount, period: periodLabel, showStrike: hasDiscount, saved, originalAmount: hasDiscount ? `$${price.original.toFixed(2)}` : null };
}
    ...
    <section id="pricing" className="py-24 lg:py-32 relative">
      ...
      <div ref={ref} className="relative max-w-5xl mx-auto px-6 lg:px-8">
        ...
        <div className="grid md:grid-cols-2 gap-6 lg:gap-8 max-w-3xl mx-auto">
          {plans.map((plan, i) => {
            const price = plan.prices[devices][billing];
            const display = getPriceDisplay(price, billing, devices);
        ...
        <motion.div
          ...
          className="flex flex-wrap items-center justify-center gap-6 sm:gap-8 mt-8 text-sm text-txt-dim"
        >
